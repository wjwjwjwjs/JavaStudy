...
public class PersonConstructor {
	...
	디폴트 생성자 : 해당 클래스에 생성자를 정의하지 않으면 아래와
		...
		자동으로 삽입된다. 지금까지 우리가 new 클래스명()과 
		같이 사용했던 문장이 바로 디폴트 생성자를 호출하여 객체를
		생성한는 것이었다.
	 */
	...
	public PersonConstructor(String name) {
		...
		용도로 사용된다. 클래스 자신, 즉 멤버를 가리킨다.
		따라서 좌측항은 멤버변수, 우측항은 매개변수가 된다. */
		this.name = name;
		age = 1;
		addr = "출처불명";
	...
	public PersonConstructor(String _name, int age, String addr) {
		...
		name = _name;
		// 둥의 이름이 동일한 경우에는 this를 사용하도록 권고한다.
		this.age = age;
		this.addr = addr;
	...
	void initialize(String name, int age, String addr) {
		...
		생성자는 인스턴스 생성시 딱 한번만 호출되고, 개발자가
		임의로 호출할 수 없다. 멤버메서드는 개발자가 원할때
		언제든 호출이 가능하므로 생성자와는 용도와 특성이 다르다.
