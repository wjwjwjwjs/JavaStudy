...
2. 그러면 클래스 외부에서는 생성자에 접근할 수 없으므로
	new를 통해 인스턴스 생성이 불가능해진다.
3. 클래스 내부에 멤버변수를 static으로 선언하여
	인스턴스를 미리 생성해 놓는다
4. genInstance() 라는 유틸리티 메서드를 통해
	참조값을 반환받아 사용한다.
5. 이 방법을 통해 인스턴스를 생성하면 메모리에
	딱 하나만 만들어지게 되어 메모리를 절약할 수 있다.
 */
...
class NoSingleTone {
	...
	만약 public으로 선언하지 않으면 클래스외부 혹은
	다른 패키지에서는 호출할 수 없어 new를 통해
	인스턴스를 생성할 수 없기 때문이다. */
	public NoSingleTone() {}
}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		생성자가 "not visible" 즉, 보이지 않는다는 에러가
		발생한다. */
