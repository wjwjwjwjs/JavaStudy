...
public class E09InstanceOf {
	...
	/*
	- 매서드 정의 시 부모타입의 매개변수로 설정하면 자식타입의
		인스턴스를 한꺼번에 받을 수 있다. 
	- Box타입으로 인수를 받게디면 자동형변환(업캐스팅) 되므로
		자식클래스의 멤버메서드를 호출할 수 없게된다.
	- 이때 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)
		이 필요하다
	- instanceof 연산자를 통해 자식타입으로 형변환이 가능한지 
	...
	상속관계가 더 깊어진다면 메서드는 점점 늘어나게되어 코드는
	더욱 더 복잡해지게 될 것이다. 따라서 부모로 자식을 참조할 수
	있는것은 개발자에게 편리함을 가져다준다.
	 */
