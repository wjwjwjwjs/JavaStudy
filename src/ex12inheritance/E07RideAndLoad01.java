...
class C extends B{
	/* rideMethod()는 오버라이딩 된 메서드로 상위클래스에
	정의된 멤버메서드와 반환형, 메서드명, 매개변수의 갯수 및
	타입 모두 동일하다. 즉, 똑같은 메서드이다. */
	@Override
	public void rideMehhod() {
		System.out.println("C의 rideMethod");
	}
	
	/* loadMethod()는 오버로딩 된 메서도로 상위클래스에 정의된
	멤버메ㅔ서드와 메서드명만 동일할 뿐 매개변수의 타입, 갯수 등이
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		ref2.loadMethod();
		ref2.loadMethod(1);
//		ref2.loadMethod(1.1); // 에러
		
		/* A타입으로는 접근범위가 A클래스까지로 제한된다 */
		ref1.loadMethod();
//		ref1.loadMethod(1); // 에러
//		ref1.loadMethod(1.1); // 에러
...
//		B refId2 = refId1; // 에러
		// C -> A 참조하므로 불가능 (자식은 부모를 참조할 수 없다)
//		C refId3 = refId1; // 에러 
		...
		진다. 즉, 부모타입을 자식타입으로 변경하는 것이다.
		이는 클래스가 상속관계에 있기 때문에 가능하고, 방식은
