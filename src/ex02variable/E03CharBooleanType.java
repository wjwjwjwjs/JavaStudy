...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		...
		char형 : 하나의 문자를 저장할 수 있는 자료형으로 값을
			저장할 때 '(Single quotation)으로 감싸서 표현한다.
			만약 "(Double quotation)으로 감싸면 문자열로 인식
			하므로 String으로 선언해야 한다.
		 */
		char ch1 = '가';
		...
		System.out.println("chStr = "+ shStr);
		...
		// 싱글쿼테이션으로 감쌋으므로 문자1(아스키코드:49)이 할당
		char ch4 = '1';
		...
		/*
		boolean 타입 : true 혹은 false 두가지의 값만 가질 수 있는
			자료형으로 선술연산(+, -), 비교연산(<, <=)에서는
		...
		/*
		&& (논리 And) : 엠퍼센트를 사용하고 양쪽 모두 참일때만
			참을 반환한다. 그 외에는 모두 거짓을 반환한다.
		|| (논리 Or) : 파이프를 사용하고, 둘중 하나만 참이어도
			참을 반환한다. 둘 다 거짓일때만 거짓을 반환한다.
		 */
		// T && F => false를 반환
		boolean bn3 = bn1 && bn2;
		// T || F => true를 반환
