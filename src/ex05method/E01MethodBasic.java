...
- 메서드 호출 후 반환값은 호출한 위치로 반환된다. 이 때 메서드는
	메모리에서 소멸된다.
...
public class E01MethodBasic {
	...
	/*
	메서드 2 : 반환값 X, 매개변수 X 형태로 정의
		=> 전달받는 값 없이 이름을 입력한 후 namePrint()
			메서드를 호출한다. 이와같이 반환값이 없는 경우에는
			반드시 void를 명시해야한다
	 */
	...
	/*
	매서드 3 : 반환값 X, 매개변수 O 형태의 정의
		=> 문자열을 인수로 전달받아 단순히 출력만한다. 
	...
	public static void main(String[] args) {
		...
		/*
		메서드를 호출할 때 함수의 원형을 그대로 기술하면 된다.
