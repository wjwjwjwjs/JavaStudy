...
- Stack은 입구각 하나만 있는 컵과 같은 형태이며, '선입후출'의 
	특성을 가진다. 또한 Stack은 CPU가 메모리의 생성 및 소멸을
	주관한다.
 */
...
//		int num = 9; // 아래 코드에서 에러가 발생됨.
		...
		/*
		main보다 좁은 지역인 if문의 블럭을 생성한 후 변수 null을
		...
		if(scope) {
			...
			int num = 5;
			System.out.println("첫번째 if문으 else지역: "+ num);
		}
		/* if문의 블럭에서 선언된 변수 num은 해당 블럭을 벗어나는
		순간 메모리에서 소멸되므로 아래에서 num을 선언할 수 있다 */
		int num = 100;
