...
public class E01If01 {
	...
	if문 형식1
		...
		- 즉, 결과값이 boolean 형으로 true/false를 반환해야한다
...
//		}
		...
		// 논리 And는 둘 다 true일때만 true를 반환한다
		if(num%2==0 && num>=10) {
		...
		/*
		조건 검사 후 실행할 문장이 1개인 경우에는 중괄호를 생략할 수 
		있다. 단, 실행문이 2개 이상이라면 한 문장만 if 문에 포함되므로
		반드시 중괄호를 기술해야 한다.
		 */
		if(num%2==0)
			System.out.println("\n" +num +"은 짝수이다");
		...
		/* 아스키코드를 모를때 구현하는 방법으로 문자 '0'과 '9'
		는 메모리에 저장시 아스키코드로 변환되므로 아래와 같은
		조건으로도 확인할 수 있다 */
		if(asciiCode>= '0' && asciiCode<= '9') {
			System.out.println("입력한 문자는 숫자는 숫자입다....");
		}
		else {
			System.out.println("입력한 문자는 숫자가 아닙니다....");
		}
