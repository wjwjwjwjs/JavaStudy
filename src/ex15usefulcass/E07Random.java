...
public class E07Random {
	...
	public static void main(String[] args) {
		...
		만약 동일한 Seed를 사용하면 항상 동일한 패턴의 난수가
		...
		/* 씨드를 부여하려면 setSeed()메서드를 사용한다.
		이때 현재시간을 밀리세컨즈 단위로 반환하는 메서드를 사용하면
		편리하다. 시간은 계속 흘러가므로 항상 새로운 Seed를
		생성할 수 있다. */
...
//		System.out.println(System.currentTimeMillis());
		...
		/* nextInt()에 인수가 없는 경우에는 음수, 양수가 
		혼용된 난수가 생성된다. 이 경우에는 Math.abs()를 통해
		양수로 변환하고 특정수를 나누는 형태로 사용해야한다. */
		System.out.println("\n# 난수생성3 : nextInt() 인수없음");
